...
struct ColorPicker3: View {
    ...
    var body: some View {
        GeometryReader { outerGeometry in
            VStack {
                ...
                makeSlider(colors: self.makeSatBrightColors(),
                ...
                makeSlider(colors: self.makeHueColors(),
    ...
    func makeSlider(colors: [Color], xPositionRatio: Double, onDragChange: @escaping (DragGesture.Value, Double) -> Color) -> some View {
    ...
    func makeSatBrightColors() -> [Color] {
        return stride(from: 0, to: 0.5, by: 0.01).map {
    ...
    func makeHueColors() -> [Color] {
        stride(from: 0, to: 1, by: 0.01).map {
            Color(hue: $0, saturation: 1, brightness: 1)
        }
    }
    
    func getSatBrightPosition() -> Double {
        if hsb.saturation == 1 {
    ...
    static func getHSB(for color: Color) -> (hue: Double, saturation: Double, brightness: Double) {
